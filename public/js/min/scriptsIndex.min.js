var $cacheAudio,$cacheProgress,playerWasPaused,playerUpdateInterval,playerUpdateTimeout,playerPreviousVolume,playerTitleOriginal,playerLoaded=playerBinded=playerHover=!1,playerInt=!0;function formatTime(a){a=Number(a);var b=Math.floor(a/3600),c=Math.floor(a%3600/60);a=Math.floor(a%3600%60);return(10>b?"0":"")+b+":"+(10>c?"0":"")+c+":"+(10>a?"0":"")+a}
...
$cacheAudio[0].addEventListener("loadedmetadata",function(){$cacheTimestamp.current.html("00:00:00");$cacheTimestamp.total.html(formatTime($cacheAudio[0].duration));$cachePlayer.removeClass("loading");playerLoaded=!0;playerUpdate();1==playerInt&&($cachePlayer.removeClass("int"),playerInt=!1,playerToggle(event));0==playerWasPaused&&($cacheAudio[0].play(),playerTitleOriginal=document.title,document.title="Playing: "+$cachePlayer.title.html())},!1);$cacheVolume.slider.on("input change",function(){$cacheAudio[0].volume=
parseInt($cacheVolume.slider[0].value)/100;playerPreviousVolume=$cacheAudio[0].volume;0==$cacheAudio[0].volume?$cacheVolume.addClass("muted"):$cacheVolume.removeClass("muted")})}function playerUpdate(){0!=playerLoaded&&(0==playerHover&&$cacheTimestamp.current.html(formatTime($cacheAudio[0].currentTime)),$cacheProgress.cover.css("width",$cacheAudio[0].currentTime/$cacheAudio[0].duration*100+"%"),1==$cacheAudio[0].ended&&clearInterval(playerUpdateInterval))}
function playerToggle(a){a.preventDefault();0==playerBinded&&playerBind();1==playerInt?($cachePlayer.addClass("loading"),$cacheAudio[0].load()):1==$cacheAudio[0].paused?(playerTitleOriginal=document.title,document.title="Playing: "+$cachePlayer.title.html(),$cacheAudio[0].play(),playerUpdate(),playerUpdateInterval=setInterval(playerUpdate,1E3),$cachePlayer.addClass("playing")):(document.title=playerTitleOriginal,$cacheAudio[0].pause(),clearInterval(playerUpdateInterval),$cachePlayer.removeClass("playing"))}
function playerChange(a,b){a.preventDefault();0==playerBinded&&(playerBind(),playerTitleOriginal=document.title);b=$(b);$cachePlayer.addClass("loading");$("html, body").animate({scrollTop:0},"slow");document.title=playerTitleOriginal;playerWasPaused=$cacheAudio[0].paused;playerLoaded=!1;$cachePlayer.title.html(b.parent().find(".title").html());$cachePlayer.date.find("span").html(b.parent().find(".date").html());$cachePlayer.title.add($cachePlayer.date).attr("href",b.parent().find(".link").attr("href"));
$cacheProgress.cover.css("width","0%");$cacheProgress.line.css("left","0%");1==playerInt&&(playerInt=!1,$cachePlayer.removeClass("int"));$cacheAudio.find("source").attr("src",b.attr("data-audio"));$cacheAudio[0].load();if(void 0!=b.attr("data-header")||void 0!=b.attr("data-background")){$cachePlayer.background.removeClass("noBackground");var c=new Image;c.src=b.attr("data-background");c.onload=function(){var a=new Image;a.src=b.attr("data-header");a.onload=function(){$cachePlayer.background.css("background-image",
"url("+c.src+")").css("background-color",b.attr("data-color"));$cachePlayer.header.css("background-image","url("+a.src+")")}}}else $cachePlayer.background.addClass("noBackground")}
