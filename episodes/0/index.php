<?php $frontpage=false; $episodepage=true; $pagetitle=$headertitle="Episode #0: Episode Title"; $headerdate="1st January 2013" ?>
	
	<?php include "../../lib/header.php"; ?>
	
	<div id="wrapper" class="episode">
		
		<span class="section">

			<h2>Description</h2>

			<p>In this debut episode of Steamchat, we discuss the podcast itself, the recently ending-E3 and the ups and downs of Valve's participation at it. We also talk about our thoughts of Left 4 Dead 2 and the lack, again, of Half-Life 2: Episode 3. We hope you enjoy the show! <!--<span class="date">&mdash; 7th June 2009</span>--></p>

		</span>

		<span class="section">

			<h2>Listen</h2>

			<div id="episode_audio_player">

				<audio id="episode_audio"><source src="steamchat_episode0.mp3" type="audio/mp3"></audio>

				<div id="episode_audio_toggle"></div>

				<div id="episode_audio_time"><span>00:00:00</span></div>

				<div id="episode_audio_progress_wrapper">

					<div id="episode_audio_progress"></div>

				</div>

			</div>

			<ul style="clear:both;">

				<li><a href="http://www.thesteamchat.com/episodes/1/steamchat_episode1.mp3">Direct MP3 Download</a></li>
				<li><a href="http://www.thesteamchat.com/steamchat_feed.xml">Podcast RSS Feed (M4A)</a></li>
				<li><a href="http://www.thesteamchat.com/steamchat_feed_mp3.xml">Podcast RSS Feed (MP3)</a></li>
				<li><a href="http://itunes.apple.com/WebObjects/MZStore.woa/wa/viewPodcast?id=320594165">iTunes Subscription</a></li>

			</ul>

		</span>
		
	</div>
	
<?php include "../../lib/footer.php"; ?>